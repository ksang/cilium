...
func (d *Daemon) missingK8sPodV1(m versioned.Map) versioned.Map {
	missing := versioned.NewMap()
	for k, v := range m {
		pod := v.Data.(*v1.Pod)
		_, exists := ipcache.IPIdentityCache.LookupByIP(pod.Status.PodIP)
		// We can only consider that a pods is missing if it doesn't exist
		...
		if !podEP.HasLabels(podFilteredLabels) {
			missing.Add(k, v)
		}
	}
	return missing
